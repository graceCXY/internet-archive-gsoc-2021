...
# OUTPUT:
## json string
proc get_wikimedia_json(language:string, source:string, citation:string, verbose = false): string = 
    
    ### build url
    var url_header = "https://" & language & "." & source & ".org/w/api.php?action=parse&text="
    ...
    ### make http request
    var client = newHttpClient(timeout = 20000, maxRedirects = 3)
    var response = client.get(url)
    if not response.status.contains("200") or response.body == "":
        sleep(15000)
        client = newHttpClient(timeout = 20000, maxRedirects = 3)
...
proc find_html_lst_from_json(response_text:string, verbose = false): seq[string] = 
    ...
    var res_json:JsonNode
    try:
        res_json = parseJson(response_text)
    ...
    var html_json_node:JsonNode 
    try:
        html_json_node = res_json["parse"]["text"]["*"]
    except:
        return @[]
...
proc autourl_exists(citation:string, language = "en", source = "wikipedia", verbose = false): bool = 
    
    var res_json = get_wikimedia_json(language, source, citation, verbose)
    if res_json == "":
        return false
    
    var html_lst = find_html_lst_from_json(res_json, verbose)
    if html_lst == @[] or html_lst.len() == 0:
        return false
...
var test_json =  get_wikimedia_json("en", "wikipedia", test_cite1)
echo test_json
...
var test_json2 =  get_wikimedia_json("tr", "wikipedia", test_cite2)
...
echo autourl_exists(test_cite1, "en", "wikipedia")

## example of having no autogenerated url
echo autourl_exists(test_cite2, "tr", "wikipedia")
